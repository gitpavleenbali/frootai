...
export default function CommunityPage(): JSX.Element {
  return (
    <Layout title="Open Source Community — FrootAI" description="FrootAI is 100% open source. Free forever. Come build the community.">
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "40px 24px 80px" }}>
        ...
        <section style={{ marginBottom: "56px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "16px" }}>
            {[
              ...
              { icon: "⭐", title: "Star and Share", detail: "Star the repo, share with your team. Community grows the ecosystem." },
            ].map((item) => (
        ...
        <section style={{ textAlign: "center", padding: "40px 24px", borderRadius: "16px", border: "2px solid rgba(0, 200, 83, 0.2)", background: "rgba(0, 200, 83, 0.03)" }}>
          <h2 style={{ fontSize: "1.2rem", fontWeight: 700, marginBottom: "20px" }}>Ready to Join?</h2>
          <div style={{ display: "flex", gap: "14px", justifyContent: "center", flexWrap: "wrap" }}>
            <Link to="/setup-guide" className={styles.glowPill} style={{ "--pill-color": "#10b981", display: "inline-block" } as React.CSSProperties}>Get Started Free →</Link>
            <Link to="/solution-plays" className={styles.glowPill} style={{ "--pill-color": "#7c3aed", display: "inline-block" } as React.CSSProperties}>Solution Plays →</Link>
            <Link href="https://github.com/gitpavleenbali/frootai" className={styles.glowPill} style={{ "--pill-color": "#eab308", display: "inline-block" } as React.CSSProperties}>⭐ Star on GitHub →</Link>
            <Link to="/learning-hub" className={styles.glowPill} style={{ "--pill-color": "#f59e0b", display: "inline-block" } as React.CSSProperties}>📚 FAI Learning Hub →</Link>
          </div>
        </section>
